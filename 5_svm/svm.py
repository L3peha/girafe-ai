...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    #distances = np.exp(- (np.sum(np.power((x_1-x_2),2))) / (2*sigma**2))### YOUR CODE HERE
    #distances = torch.pow((x_1-x_2),2) + 1
    smp1, smp2 = len(x_1), len(x_2)
    distances = torch.zeros(smp1, smp2)
    for i in range(smp1):
        for j in range(smp2):
            distances[i][j] = torch.exp(-(torch.linalg.norm(x_1[i]-x_2[j], ord=2))**2 /(2*np.power(sigma, 2)))
...
def hinge_loss(scores, labels):
    ...
    #return np.average(max(0,1 - labels * scores))### YOUR CODE HERE
